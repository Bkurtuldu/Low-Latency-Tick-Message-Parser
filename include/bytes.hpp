...
// Read exactly n bytes from stream into dst; throw on EOF
...
//Reads 2 byte big endian integer from byte array
inline uint16_t be16(const unsigned char b[2]) {
    return (uint16_t(b[0]) << 8) | uint16_t(b[1]);
}

//Reads 4 byte big endian integer from byte array
inline uint32_t be32(const unsigned char b[4]) {
    return (uint32_t(b[0]) << 24) | (uint32_t(b[1]) << 16) |
           (uint32_t(b[2]) << 8)  |  uint32_t(b[3]);
}

//Reads 8 byte big endian integer from byte array
...
//Reads 4 byte big endian signed integer from byte array
